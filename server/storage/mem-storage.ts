...
export class MemStorage implements IStorage {
  ...
  async createPrayerTimes(insertPrayerTimes: InsertPrayerTimes): Promise<PrayerTimes> {
    ...
    const prayerTimes: PrayerTimes = {
      ...insertPrayerTimes,
      id,
      location: insertPrayerTimes.location || "Beirut, Lebanon", 
      scrapedAt: new Date(),
    };
  ...
  async createNotificationSettings(insertSettings: InsertNotificationSettings): Promise<NotificationSettings> {
    ...
    ...insertSettings,
    id,
    userId: insertSettings.userId || "default", 
    fajrEnabled: insertSettings.fajrEnabled ?? true,
    dhuhrEnabled: insertSettings.dhuhrEnabled ?? false,
    asrEnabled: insertSettings.asrEnabled ?? true,
    maghribEnabled: insertSettings.maghribEnabled ?? true,
    ishaEnabled: insertSettings.ishaEnabled ?? false,
    adhanAutoPlay: insertSettings.adhanAutoPlay ?? true,
    volume: insertSettings.volume ?? "80",
    };
